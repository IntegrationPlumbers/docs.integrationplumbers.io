...
You need Enterprise Manager 13.5 or 24ai, an agent that can reach the instance on TCP, and the ability to create a login on that instance. Full list in [Prerequisites](prerequisites.md).

Have to hand: the instance address and port, and — if it is a named instance — its instance name.

## Install {#install}

Import the archive, deploy it to the management server, then to the agent that will do the monitoring. Commands are in [Install and upgrade](install-and-upgrade.md).
...
| [Alerts and thresholds](alerts-and-thresholds.md) | What will alert you, and at what values, before it does |
| [AG Failover Readiness](monitoring-pages.md#ag-failover) | If the instance is in an availability group |
...
**Target is Up but pages look sparse.** Expected in the first 24 hours. Configuration and per-database space collect daily, so anything depending on them is empty until the first collection. The Overview page works around the two slowest with a live read. See [Targets and properties](targets-and-properties.md#new-target).

**Target Down only when certificate verification is on.** Set TLS Mode to `required` briefly: if it comes Up, the problem is the certificate chain rather than the network or login. [TLS connections](tls.md) has the specifics.

**A job fails on a target that is monitoring fine.** Jobs use a different credential path. See [Jobs](jobs.md#prerequisites).

More in [Troubleshooting](troubleshooting.md).

## Related

- [Prerequisites](prerequisites.md) - what to check before you begin
- [Install and upgrade](install-and-upgrade.md) - importing and deploying the plug-in
- [Targets and properties](targets-and-properties.md) - what each target property means
- [Credentials](credentials.md) - the monitoring login and its grants
- [Monitoring pages](monitoring-pages.md) - what each console page is telling you
- [Alerts and thresholds](alerts-and-thresholds.md) - what will alert you, and at what values
- [Troubleshooting](troubleshooting.md) - when a step above does not go as described
