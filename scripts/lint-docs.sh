...
# Exit 1 on any finding except "pending image" when --allow-missing-images is set.
set -u
cd "$(dirname "$0")/.."
ALLOW=0
...
FORBID='\[INTERNAL|INTERNAL:|Winter|24\.1\.2|\bPRD\b|roadmap|Query Advisor|Plan Capture readiness|one-click|Disk Sort|Hash Batches|Expensive Node|Large Offset|arrives in|next release|coming soon|planned for|\bTBD\b|\bTODO\b|encrypted at rest|Lorem ipsum'
