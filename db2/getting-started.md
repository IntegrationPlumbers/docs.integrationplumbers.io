...
emcli deploy_plugin_on_server -plugin=ip.em.xdbb:24.1.9.7.0
emcli get_plugin_deployment_status -plugin=ip.em.xdbb      # wait for Success
emcli deploy_plugin_on_agent -plugin=ip.em.xdbb:24.1.9.7.0 -agent_names="<agent_host>:<agent_port>"
```

Use `13.5.9.2.0` in place of `24.1.9.7.0` on Enterprise Manager 13.5. On 24ai, `deploy_plugin_on_server` takes `-dbUser`/`-dbPassword` for the repository account rather than `-sys_password`.
